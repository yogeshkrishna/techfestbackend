...
**1 — Open Git Bash** in the project folder (right-click → "Git Bash Here" or open Git Bash and navigate):
```bash
cd /c/Users/yokri/Desktop/webdevproj
```

**2 — Configure your identity** (skip if already done):
```bash
git config --global user.name "Your Name"
git config --global user.email "your-email@example.com"
```

**3 — Initialise git and make first commit:**
```bash
git init
git add .
git commit -m "Initial commit: Tech Fest backend (Phases 0–8)"
```

**4 — Create a repo on GitHub:**
- Go to [github.com/new](https://github.com/new)
- Name it (e.g. `techfest-backend`), set to private, **do not** tick "Add README" (you already have one)
- Click Create

**5 — Push to GitHub:**
```bash
git remote add origin https://github.com/YOUR_USERNAME/techfest-backend.git
git branch -M main
git push -u origin main
```

You'll be prompted for your GitHub username and a **Personal Access Token** (not your password — [create one here](https://github.com/settings/tokens) with `repo` scope).
...
### What's included / excluded from git

| Included ✅ | Excluded ❌ (in `.gitignore`) |
|---|---|
| All source code (`src/`) | `node_modules/` |
| `prisma/schema.prisma` | `.env` (contains secrets) |
| `prisma/migrations/` | `dist/`, `build/` |
| `prisma/seed.js` | `*.log` files |
| `postman_collection.json` | `.DS_Store`, `Thumbs.db` |
| `README.md`, `SCALE.md` | Editor files (`.vscode/`, `.idea/`) |
| `.env.example` (no real secrets) | `coverage/` |
| `package.json`, `package-lock.json` | |

> **For senior devs cloning this:** Run `npm install` to restore `node_modules`. Copy `.env.example` to `.env` and fill in your own credentials. Run `npx prisma migrate deploy` to create the database schema. Run `npm run prisma:seed` to create the HOST account. Then `npm run dev` to start.
